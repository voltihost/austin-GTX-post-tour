            ...
            <Card className="shadow-xl bg-white border-0">
              ...
              <CardContent className="px-4 md:px-6">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
                  {teamMembersData.map(member => <button key={member.name} onClick={() => handleMemberClick(member)} className="group p-3 md:p-4 rounded-xl border-2 border-gray-100 hover:border-water-primary hover:bg-water-primary/5 transition-all duration-200 text-center cursor-pointer active:scale-95">
          ...
          <CarouselItem>
            <Card className="shadow-xl bg-white border-0">
              ...
              <CardContent className="space-y-4 px-4 md:px-6">
                {selectedMember && <div className="text-center mb-4 md:mb-6 p-4 bg-water-primary/10 rounded-xl">
                ...
                <div className="space-y-3">
                  {selectedMember?.cashApp && <Button onClick={() => handlePaymentMethod('cashapp')} className="w-full bg-water-primary hover:bg-water-deep text-white py-3 md:py-4 text-base md:text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all">
                      <DollarSign className="w-4 h-4 md:w-5 md:h-5 mr-3" />
                      Tip via CashApp
                      <ArrowRight className="w-4 h-4 md:w-5 md:h-5 ml-auto" />
                    </Button>}

                  {selectedMember?.venmo && <Button onClick={() => handlePaymentMethod('venmo')} className="w-full bg-water-primary hover:bg-water-deep text-white py-3 md:py-4 text-base md:text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all">
                      <DollarSign className="w-4 h-4 md:w-5 md:h-5 mr-3" />
                      Tip via Venmo
                      <ArrowRight className="w-4 h-4 md:w-5 md:h-5 ml-auto" />
                    </Button>}

                  {selectedMember?.paypal && <Button onClick={() => handlePaymentMethod('paypal')} className="w-full bg-purple-accent hover:bg-purple-accent/80 text-white py-3 md:py-4 text-base md:text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all">
                      <DollarSign className="w-4 h-4 md:w-5 md:h-5 mr-3" />
                      Tip via PayPal
                      <ArrowRight className="w-4 h-4 md:w-5 md:h-5 ml-auto" />
                    </Button>}

                  {selectedMember?.zelle && <div className="w-full">
                      <ZelleDropdown memberName={selectedMember.name} zelleNumber={selectedMember.zelle} />
                    </div>}

                  {selectedMember && !selectedMember.cashApp && !selectedMember.venmo && !selectedMember.paypal && !selectedMember.zelle && <div className="text-center py-6 md:py-8">
          ...
          <CarouselItem>
            <Card className="shadow-xl bg-white border-0">
              ...
              <CardContent className="text-center px-4 md:px-6">
                {selectedMember && <div className="mb-6 md:mb-8">
